...
class SequenceGenerator(object):
    ...
    def generate(
        ...
        **kwargs
        ...
        bert_outs, _ = model.models[0].bert_encoder(bertinput, output_all_encoded_layers=True,  attention_mask= ~ bert_encoder_padding_mask) # attention_mask= 1. - bert_encoder_padding_mask) # RuntimeError: Subtraction, the `-` operator, with a bool tensor is not supported. If you are trying to invert a mask, use the `~` or `bitwise_not()` operator instead. 
